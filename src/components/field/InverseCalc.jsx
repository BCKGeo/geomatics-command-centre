  ...
  const renderSvg = () => {
    ...
    return (
      <svg viewBox={`0 0 ${sz} ${sz}`} width="100%" style={{ maxWidth: 220 }} preserveAspectRatio="xMidYMid meet">
        ...
        <text x={ax + 6} y={ay - 48} fill={B.textDim} fontSize={9} fontFamily={B.font}>N</text>
        {/* Azimuth arc */}
        <path d={`M ${northAx} ${northAy} A ${arcR} ${arcR} 0 ${largeArc} 1 ${fwdArcX} ${fwdArcY}`} fill="none" stroke={B.acc} strokeWidth={1.5} />
        <text x={(northAx + fwdArcX) / 2 + 8} y={(northAy + fwdArcY) / 2} fill={B.acc} fontSize={9} fontFamily={B.font}>{result.fwdAzimuth.toFixed(1)}{"\u00B0"}</text>
        {/* Connection line */}
        <line x1={ax} y1={ay} x2={bx} y2={by} stroke={B.priBr} strokeWidth={1.5} strokeDasharray="5,3" />
        {/* Distance label */}
        <text x={(ax + bx) / 2 + 6} y={(ay + by) / 2 - 6} fill={B.text} fontSize={9} fontFamily={B.font}>{result.distance >= 1000 ? (result.distance / 1000).toFixed(1) + " km" : result.distance.toFixed(1) + " m"}</text>
        {/* Points */}
        <circle cx={ax} cy={ay} r={4} fill={B.priBr} />
        <text x={ax - 12} y={ay + 14} fill={B.priBr} fontSize={10} fontWeight="700" fontFamily={B.font}>A</text>
  ...
  const dmsPoint = (label, latD, latM, latS, latDir, setLatD, setLatM, setLatS, setLatDir, lonD, lonM, lonS, lonDir, setLonD, setLonM, setLonS, setLonDir) => (
    <div style={{ marginBottom: 6 }}>
      ...
      <div style={{ display: "flex", gap: 4, alignItems: "center", marginBottom: 4, flexWrap: "wrap" }}>
        ...
        <input value={latS} onChange={e => setLatS(e.target.value)} style={{ ...dmsInp, width: 60 }} /><span style={{ fontSize: 11, color: B.textDim }}>{"\u2033"}</span>
      ...
      <div style={{ display: "flex", gap: 4, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <input value={lonS} onChange={e => setLonS(e.target.value)} style={{ ...dmsInp, width: 60 }} /><span style={{ fontSize: 11, color: B.textDim }}>{"\u2033"}</span>
          ...
          {mode === "dd" ? (
            <div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center", flexWrap: "wrap" }}>
                <label style={{ fontSize: 11, color: B.textMid }}>Lat</label><input value={aLat} onChange={e => setALat(e.target.value)} style={{ ...inp, width: 120 }} />
                <label style={{ fontSize: 11, color: B.textMid }}>Lon</label><input value={aLon} onChange={e => setALon(e.target.value)} style={{ ...inp, width: 120 }} />
              </div>
              <div style={{ fontSize: 10, color: B.textDim, marginBottom: 3, fontWeight: 600 }}>Point B</div>
              <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center", flexWrap: "wrap" }}>
                <label style={{ fontSize: 11, color: B.textMid }}>Lat</label><input value={bLat} onChange={e => setBLat(e.target.value)} style={{ ...inp, width: 120 }} />
                <label style={{ fontSize: 11, color: B.textMid }}>Lon</label><input value={bLon} onChange={e => setBLon(e.target.value)} style={{ ...inp, width: 120 }} />
              </div>
            </div>
          ) : (
            <div>
