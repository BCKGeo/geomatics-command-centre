        ...
        <div style={{display:"grid",gap:2}}>
          {section.links.map(l=>(
            <a key={l.u} href={l.u} target="_blank" rel="noopener noreferrer" style={linkStyle}
              ...
              <span>{l.n}</span><span style={{color:B.textDim,fontSize:11}}>{"\u2192"}</span>
            </a>
          ))}
        </div>
      </div>
        ...
        <div style={{maxWidth:720}}>
          <div style={{...cardStyle,marginBottom:12}}>
            ...
            <div style={{fontSize:12,color:B.text,lineHeight:1.7}}>
              ...
              <p style={{marginBottom:12}}>The dashboard aggregates real-time space weather data from <b style={{color:B.text}}>NOAA SWPC</b> (G/S/R scales, Kp index, solar wind) because geomagnetic conditions directly affect GNSS accuracy, RTK reliability, and PPP convergence times. Local weather is pulled from <b style={{color:B.text}}>Open-Meteo</b>. Magnetic declination is computed from the <b style={{color:B.text}}>World Magnetic Model 2025</b> (WMM2025), the current-epoch global geomagnetic reference co-produced by NOAA and the British Geological Survey, valid 2025 to 2030.</p>
              <p style={{marginBottom:12}}>Each tab organizes curated links to authoritative sources: Transport Canada RPAS regulations and NOTAMs for flight operations, NRCan geodetic tools (CSRS-PPP, TRX, GPS-H) for post-processing and datum transformations, BC provincial data services (BCGW, iMapBC, ParcelMap BC, LTSA) for GIS workflows, and professional standards bodies (ASTTBC, ABCLS, ACLS, CIG) for regulatory reference.</p>
              <p style={{marginBottom:0,color:B.textMid}}>The Field Tools tab includes a WMM2025 magnetic declination calculator, coordinate converter (DD/DMS/UTM/MTM), combined scale factor computation, and common unit conversions used in survey fieldwork. The Glossary provides definitions for 60+ domain-specific terms organized by discipline.</p>
            </div>
          </div>
          <div style={cardStyle}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            ...
            <div style={{display:"grid",gap:2}}>
              {[{n:"NOAA SWPC",u:"https://www.swpc.noaa.gov/",d:"Space Weather Prediction Center"},{n:"Open-Meteo",u:"https://open-meteo.com/",d:"Free weather API"},{n:"WMM2025",u:"https://www.ncei.noaa.gov/products/world-magnetic-model",d:"World Magnetic Model 2025"}].map(l=>(
                <a key={l.u} href={l.u} target="_blank" rel="noopener noreferrer"
                  style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",textDecoration:"none",color:B.text,fontSize:12,background:"transparent",transition:"all .12s",border:"1px solid transparent",borderRadius:5}}
